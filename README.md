...
1. Within the SSH session to the Azure VM, run the following commands to build a custom image based on the **azhop-centos79-v2-rdma-gpgpu.json** configuration file.
...
1. On the lab VM, navigate back to **Azure Portal** and serach for **Azure compute galleries** then select it.

    ![Azue Compute gallery](../main/media/AcG.png)

1. On the **Azure compute galleries** blade, select the entry which name starts with the prefix **azhop**.
